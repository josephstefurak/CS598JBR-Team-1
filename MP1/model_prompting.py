...
def post_process(prompt, response, entry_point):
    text = response

    # Strip markdown fences (the instruct model wraps everything)
    blocks = re.findall(r"```(?:python|py)?\s*\n(.*?)(?:```|$)", text, re.DOTALL)
    if blocks:
        text = next((b for b in blocks if f"def {entry_point}" in b), blocks[0])

    # Model redefined the whole function: append at top level to override the stub
    if re.search(rf"^def {entry_point}\s*\(", text, re.MULTILINE):
        candidate = "\n" + text.strip("\n") + "\n"
        if _valid(prompt, candidate):
            return candidate

    # Body continuation: cut at the first top-level line (drops trailing extra functions)
    body = []
    for line in text.split("\n"):
        if line.strip() and not line[0].isspace() and any(l.strip() for l in body):
            break
        body.append(line)
    body = "\n".join(body).rstrip() + "\n"

    # Indentation repair
    for candidate in (body, textwrap.indent(textwrap.dedent(body), "    "), "    " + body.lstrip()):
        if _valid(prompt, candidate):
            return candidate
    return body
...
def prompt_model(dataset, model_name = "deepseek-ai/deepseek-coder-6.7b-base", quantization = True):
    ...
    for case in dataset:
        ...
        response_processed = post_process(prompt, response, case["entry_point"])
